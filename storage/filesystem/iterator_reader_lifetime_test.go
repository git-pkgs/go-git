	...
	for _, format := range []string{"sha1", "sha256"} {
		for _, mapped := range []bool{false, true} {
			...
			t.Run(name, func(t *testing.T) {
				...
				infoStorage := NewStorage(osfs.New(dir, opts...), cache.NewObjectLRUDefault())
				t.Cleanup(func() { require.NoError(t, infoStorage.Close()) })
				infoIter, err := infoStorage.IterObjectInfos(plumbing.BlobObject)
				require.NoError(t, err)
				info, err := infoIter.Next()
				require.NoError(t, err)
				_, err = infoIter.Next()
				require.ErrorIs(t, err, io.EOF)
				infoIter.Close()
				infoObj, err := infoStorage.EncodedObjectFromInfo(info)
				require.NoError(t, err)
				infoReader, err := infoObj.Reader()
				require.NoError(t, err)
				defer func() { require.NoError(t, infoReader.Close()) }()
				got, err = io.ReadAll(infoReader)
				require.NoError(t, err)
				require.Equal(t, content, got)
			})
		}
